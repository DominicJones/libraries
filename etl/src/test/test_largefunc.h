...
#ifndef _test_largefunc_h_
#define _test_largefunc_h_


#include "largefunc.h"


namespace largefunc_data
{
...
void largefunc_primal_reference(int n = 1)
{
  name = "largefunc-pri-ref";

  namespace data = largefunc_data;
  data::reset_primal();
...
void largefunc_adjoint(int n = 1)
{
  name = "largefunc-adj";
...
void largefunc_adjoint_tapenade(int n = 1)
{
  name = "largefunc-adj-tapenade";
...

  {
    for (int it = 0; it != n; ++it)
    {
      data::update_primal(it);
      data::f_adj = 1;
      largefunc_adjoint_tapenade(double(data::alpha), &data::alpha_adj,
...
void test_largefunc(int r_size, int t_size)
{
  timers.clear();
  ...
  {
    largefunc_primal_reference(r_size);
    largefunc_adjoint(r_size);
    largefunc_adjoint_tapenade(r_size);
  ...
  double t_ref = 1;
  {
    auto timer = timers.find("largefunc-pri-ref");
  ...
  std::cout << " reference | " << t_ref << std::endl;
}


#endif // _test_largefunc_h_
